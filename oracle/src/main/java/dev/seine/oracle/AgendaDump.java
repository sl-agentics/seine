...
/**
 * D-372 graft: AccDump + agenda MATCH lifecycle events (created /
 * cancelled / fired) + the epoch action-insert op. Diagnostic only.
 */
public final class AgendaDump {
    ...
    public static void main(String[] args) throws Exception {
        ...
        session.addEventListener(new DefaultAgendaEventListener() {
            ...
            public void matchCreated(org.kie.api.event.rule.MatchCreatedEvent event) {
                ...
                sb.append(event.getMatch().getRule().getName()).append(' ');
            ...
            public void matchCancelled(org.kie.api.event.rule.MatchCancelledEvent event) {
                StringBuilder sb = new StringBuilder("MATCH-CANCELLED ");
                sb.append(event.getMatch().getRule().getName()).append(' ');
                for (Object o : event.getMatch().getObjects()) sb.append(short_(o)).append(" | ");
                System.out.println(sb);
            }
        ...
        for (JsonNode epoch : scenario.path("epochs")) {
            ...
            for (JsonNode action : epoch.path("actions")) {
                ...
                if (op.equals("advance")) {
                    ...
                    System.out.println("-- ACTION-INSERT " + action.path("fields"));
                } else {
