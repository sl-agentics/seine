...
public abstract class AbstractHashTable
    ...
    protected void resize(final int newCapacity) {
        ...
        // SEINE DIAGNOSTIC SHADOW (>96-key resize recon): trace-only,
        // zero semantic change. Guarded by SEINE_RESIZE_TRACE.
        final boolean seineTrace = System.getenv("SEINE_RESIZE_TRACE") != null;
        if (seineTrace) {
            StringBuilder sb = new StringBuilder("RESIZE ").append(oldCapacity)
                    .append("->").append(newCapacity).append(" size=").append(this.size)
                    .append(" this=").append(getClass().getSimpleName()).append(" OLD:");
            for (int i = 0; i < oldTable.length; i++) {
                int n = 0;
                for (TupleList e = oldTable[i]; e != null; e = e.getNext()) n++;
                if (n >= 2) {
                    sb.append(" b").append(i).append('[');
                    for (TupleList e = oldTable[i]; e != null; e = e.getNext())
        ...
        if (seineTrace) {
            StringBuilder sb = new StringBuilder("RESIZE NEW:");
            for (int i = 0; i < newTable.length; i++) {
                int n = 0;
                for (TupleList e = newTable[i]; e != null; e = e.getNext()) n++;
                if (n >= 2) {
                    sb.append(" b").append(i).append('[');
                    for (TupleList e = newTable[i]; e != null; e = e.getNext())
                        sb.append(e.hashCode()).append(' ');
                    sb.append(']');
                }
            }
