/*
 * SEINE ORACLE PATCH (D-163) — 9.44.0.Final+p1.
 *
 * This file is org.drools.core.phreak.PhreakAccumulateNode from Apache
 * Drools 9.44.0.Final (Apache-2.0; original license header below), with
 * EXACTLY ONE semantic change: the two `isDirty = ...` assignments in
 * doLeftUpdatesProcessChildren become `isDirty |= ...` — the upstream
 * fix for the stale-min/max extremum defect that Seine reported
 * (apache/incubator-kie-issues#2366) and Drools merged verbatim
 * (apache/incubator-kie-drools#6796, commit 275baf9c, 2026-07-08;
 * authored by Mario Fusco). No Drools release carries the fix, so the
 * oracle vendors it here: this class shadows the drools-core jar's copy
 * via classpath order (oracle/target/classes precedes the jars — see
 * harness/src/oracle.rs). Scope rule (DECISIONS D-163): only
 * upstream-MERGED fixes for defects Seine itself reported and
 * quarantined under the D-093 doctrine may be vendored this way.
 */
/*
