...
| `KieSession` in-place reset (paged-batch lifecycle) | **IMPLEMENTED** — D-104 (10-probe ladder, reset-drawing fuzz campaign 3x1000 clean) | (impl-only API: `StatefulKnowledgeSessionImpl.reset()`; no upstream test class — the ladder is the acceptance suite) | Clears WM/agenda/TMS/clock/handles, keeps the KieBase — mirrored exactly, incl. the listener-drop quirk (measured) and InitialFact re-creation. Python: `Session.reset()`. |
| `collectList`/`collectSet` accumulate functions + `groupby` (leading position) | **IMPLEMENTED** — D-108 (16-pin ladder; counted-set semantics; SetCollection canonical order; 30k fuzz clean) | m.i `AccumulateTest` (behavioral reference); groupby verified against the 9.44 DRL-text surface directly | Set iteration order is raw JVM HashSet — canonicalized SORTED both sides (D-052/D-090 pattern). Joined-position groupby + query-side aggregation = the next slice, walled loudly meanwhile. |
