...
| RHS arithmetic in action args (`modify($c){ setN($c.getN() + 1) }`, `insert(new T($x + 1))`, closed grammar) | P2 | m.i `MVELTest`/`Misc2Test` modify-counter methods (closed-grammar RHS subset; exact sweep at arc-open) | D-229: found by external review in use — the standard counter/running-total idiom. Engine RHS grammar today is `Lit`/`Var`/`Getter`, no arithmetic (the salience evaluator is the only arithmetic machinery). Needs: RHS grammar + evaluator (Java long-wrap per the D-220 convention, int/float promotion) + oracle probes + full battery. The authoring layer walls it cleanly meanwhile (D-229 message: `acc_sum` or Python-side `update()`). Same PREREQ as D-061: the D-076 TMS cascade must go iterative first — arithmetic + `insertLogical` = self-feeding value chains no longer bounded by rule count. |
