...
| RHS arithmetic in action args (`modify($c){ setN($c.getN() + 1) }`, `insert(new T($x + 1))`) — the counter/running-total idiom | **RHS mutation-with-computation is imperative programming inside the consequence** — a loop written as a rule, whose semantics live in fire ordering, property-reactivity masks, and `fire_limit` instead of in the match. The declarative forms are strictly better and already certified: running values are LHS `accumulate` (recomputed, order-invariant); externally computed values enter as facts or via Python-side `update()` between fires (the two-pass pattern). Landing it would also unlock self-feeding value chains (`T(n)` justifying `T(n+1)`) that today's rule-count-bounded TMS cascade is not built for. The D-229 authoring wall names both remedies; raw-DRL keeps the parse wall. Constraint arithmetic on the LHS (declarative filtering) remains ROADMAP §2 (D-061). |
