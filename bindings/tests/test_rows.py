...
def test_fact_instances_end_to_end():
    ...
    res = seine.run(_rules(), {Person: people, Flagged: []})
...
def test_dict_rows_and_plain_objects():
    ...
    s = seine.Session(_rules(), {Person: people_dicts, Flagged: []})
...
def test_insert_row_object_and_handles():
    s = seine.Session(_rules(), {Person: [Person("ada", 36, 91.5)], Flagged: []})
...
def test_empty_row_list_with_fact_key_declares_type():
    ...
    res = seine.run(_rules(), {Person: [Person("ada", 36, 91.5)], Flagged: []})
    assert res.fired == 1
...
def test_none_in_row_object_still_rejected():
    with pytest.raises(ValueError, match="None"):
        seine.run(_rules(), {Person: [Person("ada", None, 1.0)], Flagged: []})


def test_missing_field_in_dict_row():
    with pytest.raises(ValueError, match="missing field"):
        seine.run(_rules(), {Person: [{"name": "ada", "age": 36}], Flagged: []})
...
def test_mixed_rows_share_schema_order():
    ...
    res = seine.run(_rules(), {Person: rows, Flagged: []})
    assert res.fired == 2
...
def test_row_sugar_equals_column_path():
    ...
    r1 = seine.run(_rules(), {Person: people_rows, Flagged: []})
    r2 = seine.run(_rules(), {Person: people_cols, Flagged: []})
